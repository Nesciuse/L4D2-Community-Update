...
local function multi_func_criteria(functors) {
    return [
        function(query) {
            query.setdelegate(case_insensitive)
            foreach(func in functors) {
                if(!func(query)) {
                    return false
                }
            }
...
local function single_func_criterion(func) {
    return [
        function(query) {
            query.setdelegate(case_insensitive)
            return func(query)
...
if(Entities.FindByName(null, "base_weight_crit_global") == null) {
    SpawnEntityFromTable("env_global", {
        targetname = "base_weight_crit_global",
        globalstate = "base_weight_crit",
        initialstate = 1,
        spawnflags = 1
    });
}
local weight1 = ["base_weight_crit", 1]
local function add_weight(weight, criteria) {
    if(weight > 0) {
        criteria.extend(array(weight, weight1))
    }
    return criteria
}

local function criteria_fixup(crits) {
    local functors = [], statics = [];
    ...
    local criteria = statics;
    local l = functors.len();
    if(l == 1) {
        criteria.append(single_func_criterion(functors[0]));
    }
    else if(l > 1) {
        criteria.append(multi_func_criteria(functors));
        add_weight(l-1, criteria);
    }
    return criteria;
}

local no_params = RGroupParams();
function rr_ProcessRules( rulesarray )
{
	...
	foreach( rule in rulesarray )
	{
		if(!("group_params" in rule)) {
			rule.group_params <- no_params;
		}
		rule.criteria = criteria_fixup(rule.criteria);
